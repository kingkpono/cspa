...
class CassObjectController extends Controller
{
    ...
    public  function getDue(Request $request)
    {
        ...
        return response()->json(CassObject::whereRaw('due_year <="'.$request->due_year.'" AND due_month <="'.$request->due_month.'"')->with('client')->with('serviceType')->with('cassType')->with('user')->get(),200);
    }
